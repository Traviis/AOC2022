...
pub type InputType = Vec<Rucksack>;
pub type OutputType = usize;

pub struct Rucksack {
    //I thought I would need to count the duplicates, I guess not.
...
impl Rucksack {
    fn new(ln: &str) -> Self {
        ...
        cs.by_ref().take(len / 2).for_each(|c| *first.entry(c).or_insert(0) += 1);
    ...
    fn find_duplicates(&self) -> Vec<char> {
        let first_set = self.first.iter().map(|(k,_)| *k).collect::<HashSet<char>>();
        let second_set = self.second.iter().map(|(k,_)| *k).collect::<HashSet<char>>();
    ...
    fn find_item_in_three(&self, second: &Self, third: &Self) -> Option<char> {
        let first_two = self.get_combined_set().intersection(&second.get_combined_set()).map(|c| *c).collect::<HashSet<char>>();
        let final_set = first_two.intersection(&third.get_combined_set()).map(|c| *c).collect::<Vec<_>>();
...
fn day3_parse(input: &str) -> InputType {
    input.split("\n").map(|line| Rucksack::new(line)).collect()
}
...
pub fn part1(input: &InputType) -> OutputType {
    input
        .iter()
        .map(|r| r.find_duplicates()
            .iter()
            .map(|c| convert_char_to_priority(c)).sum::<usize>()
        )
        .sum::<usize>()
}
